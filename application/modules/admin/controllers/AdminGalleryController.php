<?php
class Admin_AdminGalleryController extends Zendvn_Controller_Action {
	...
	public function changeLockStatusAction() {
		$tblGallery = new Admin_Model_Gallery();
		$tblGallery->changeLockStatus($this->_arrParam,array('task'=>'admin-gallery-change-lock-status'));
		$this->_redirect($this->_actionMain);
	...
	public function deleteAction() {
		...
		if (isset($this->_arrParam['id'])) {
			...
			$tblGallery = new Admin_Model_Gallery();
			$item = $tblGallery->getItem($this->_arrParam,array('task'=>'admin-gallery-info'));
			$this->view->Item = $item;
		...
		if ($this->_request->isPost()) {
			$tblGallery = new Admin_Model_Gallery();
			$tblGallery->deleteItem($this->_arrParam,array('task'=>'admin-gallery-delete'));
			$this->_redirect($this->_actionMain);
		}
	}
	...
	public function multiDeleteAction() {
		...
		$this->view->Title = $this->_translate->_('admin-gallery-deletemulti-title');
		$this->view->headTitle($this->view->Title,true);
		...
		if ($this->_request->isPost()) {
			if (!isset($this->_arrParam['cid'])) {
				...
				if (!isset($this->_arrParam['delConfirm'])) {
					...
					$this->view->cid = $cid;
					$tblGallery = new Admin_Model_Gallery();
					$items = $tblGallery->listItem($this->_arrParam,array('task'=>'admin-gallery-list-from-array'));
					...
					$cid = $this->_arrParam['cid'];
					$tblGallery = new Admin_Model_Gallery();
					$tblGallery->deleteItem($this->_arrParam,array('task'=>'admin-gallery-multi-delete'));
					$this->_redirect($this->_actionMain);
				}
			}
		}
	...
	public function publishAction() {
		...
		if (!is_array($lockStatus)) {
			...
			if ($lockStatus == 1) {
				$alertLockStatus = $this->_translate->_('admin-gallery-publish-warning');
				echo "<script>alert('" . $alertLockStatus ."'); window.location = '" . $this->_actionMain . "'</script>";
			} else {
				$tblGallery->publish($this->_arrParam);
				$this->_redirect($this->_actionMain);
			}
			...
			foreach ($lockStatus as $key=>$val) {
				if ($val['lock_status'] == 1) {
					$flag = false;
					$alertLockStatus = $this->_translate->_('admin-gallery-publishmulti-warning');
					echo "<script>alert('" . $alertLockStatus ."'); window.location = '" . $this->_actionMain . "'</script>";
				}
			}
			if ($flag == true) {
