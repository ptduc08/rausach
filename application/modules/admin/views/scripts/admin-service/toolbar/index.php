<?php 
	...
	//------------- nut Featured Services - kieu submit
	$linkFeaturedServices = $this->baseUrl($this->currentController . '/change-featured-service/type/active');
	$btnFeaturedServices = $this->cmsButton($this->translate('button-featured-service'),$linkFeaturedServices,
			$this->imgUrl . '/toolbar/icon-32-featuredservice.png','submit');
	//------------- nut Un Featured Services - kieu submit
	$linkUnFeaturedServices = $this->baseUrl($this->currentController . '/change-featured-service/type/inactive');
	$btnUnFeaturedServices = $this->cmsButton($this->translate('button-unfeatured-service'),$linkUnFeaturedServices,
			$this->imgUrl . '/toolbar/icon-32-unfeaturedservice.png','submit');
	...
	switch ($this->arrParam['action']) {
		case 'index':
			$stnBtn = $btnAddNew . ' ' . $btnDeleteItems . ' ' . $btnSortItems . ' ' .
					...
					//$btnFeaturedServices . ' ' . $btnUnFeaturedServices . ' ' .
