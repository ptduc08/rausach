<?php
class Admin_Form_ValidateProjectCategory {
	...
	public function validate($arrParam = array(), $options = null) {
		...
		switch ($arrParam['action']) {
			case 'add':
				$options = array('table'=>'project_category','field'=>'category_name');
				break;
			case 'edit':
				$clause = ' id != ' . $arrParam['id'];
				$options = array('table'=>'project_category','field'=>'category_name','exclude'=>$clause);
				break;
			case 'sort':
				$clause = ' id != ' . $arrParam['id'];
				$options = array('table'=>'project_category','field'=>'category_name','exclude'=>$clause);
	...
	public function validateDelete($arrParam = array(), $options = null) {
		...
		if ($options['task'] == 'validate-delete-projectcategory') {
			$membercount = $tblProjectCat->getItem($arrParam,array('task'=>'count-projectcategory-members'));
			if ($membercount > 0) {
				$thisProjectCat = $tblProjectCat->getItem($arrParam,array('task'=>'admin-projectcategory-info'));
				$category_name = $thisProjectCat['category_name'];
				$this->_messagesError = "You can't delete project category \"" . $category_name . "\". There're still "
											. $membercount . " projects of this category!";
			}
		}
		
		//----- validate cho truong hop xoa nhieu category
		if ($options['task'] == 'validate-delete-multi-projectcategory') {
			if (count($arrParam['cid'] > 0)) {
				...
				foreach ($cid as $key=>$val) {
					...
					$membercount = $tblProjectCat->getItem(array('id'=>$val),array('task'=>'count-projectcategory-members'));

					if ($membercount > 0) {
						$thisProjectCat = $tblProjectCat->getItem(array('id'=>$val),array('task'=>'admin-projectcategory-info'));
						$category_name = $thisProjectCat['category_name'];
						$this->_messagesError[$val] = "You can't delete project category \"" . $category_name . "\". There're still "
													. $membercount ." projects of this category!";
					} else {
