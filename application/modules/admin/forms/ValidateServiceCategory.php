<?php
class Admin_Form_ValidateServiceCategory {
	...
	public function validate($arrParam = array(), $options = null) {
		...
		switch ($arrParam['action']) {
			case 'add':
				$options = array('table'=>'service_category','field'=>'category_name');
				break;
			case 'edit':
				$clause = ' id != ' . $arrParam['id'];
				$options = array('table'=>'service_category','field'=>'category_name','exclude'=>$clause);
				break;
			case 'sort':
				$clause = ' id != ' . $arrParam['id'];
				$options = array('table'=>'service_category','field'=>'category_name','exclude'=>$clause);
	...
	public function validateDelete($arrParam = array(), $options = null) {
		...
		if ($options['task'] == 'validate-delete-servicecategory') {
			$servicemembercount = $tblServiceCat->getItem($arrParam,array('task'=>'count-servicecategory-members'));
			if ($servicemembercount > 0) {
				$thisServiceCat = $tblServiceCat->getItem($arrParam,array('task'=>'admin-servicecategory-info'));
				$category_name = $thisServiceCat['category_name'];
				$this->_messagesError = "You can't delete this category \"" . $category_name . "\". There're still "
											. $servicemembercount . " member of this category!";
			}
			$childCategoryCount = $tblServiceCat->getItem($arrParam,array('task'=>'count-servicecategory-childcategory'));
		...
		if ($options['task'] == 'validate-delete-multi-servicecategory') {
			if (count($arrParam['cid'] > 0)) {
				...
				foreach ($cid as $key=>$val) {
					...
					$servicemembercount = $tblServiceCat->getItem(array('id'=>$val),array('task'=>'count-servicecategory-members'));
					$childCategoryCount = $tblServiceCat->getItem(array('id'=>$val),array('task'=>'count-servicecategory-childcategory'));
					
					$deleteFlag = true;
					if ($servicemembercount > 0) {
						$thisserviceCat = $tblServiceCat->getItem(array('id'=>$val),array('task'=>'admin-servicecategory-info'));
						$category_name = $thisserviceCat['category_name'];
						$this->_messagesError[$val] = "You can't delete this category \"" . $category_name . "\". There're still "
													.$servicemembercount ." member of this category!<br/>";
						$deleteFlag = false;
					}
					if ($childCategoryCount > 0) {
						$thisServiceCat = $tblServiceCat->getItem(array('id'=>$val),array('task'=>'admin-servicecategory-info'));
