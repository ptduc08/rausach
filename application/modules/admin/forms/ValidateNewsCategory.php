<?php
class Admin_Form_ValidateNewsCategory {
	...
	public function validate($arrParam = array(), $options = null) {
		...
		switch ($arrParam['action']) {
			case 'add':
				$options = array('table'=>'news_category','field'=>'category_name');
				break;
			case 'edit':
				$clause = ' id != ' . $arrParam['id'];
				$options = array('table'=>'news_category','field'=>'category_name','exclude'=>$clause);
				break;
			case 'sort':
				$clause = ' id != ' . $arrParam['id'];
				$options = array('table'=>'news_category','field'=>'category_name','exclude'=>$clause);
	...
	public function validateDelete($arrParam = array(), $options = null) {
		...
		//----- validate cho truong hop xoa mot news category
		if ($options['task'] == 'validate-delete-newscategory') {
			$newsmembercount = $tblNewsCat->getItem($arrParam,array('task'=>'count-newscategory-members'));
			if ($newsmembercount > 0) {
				$thisNewsCat = $tblNewsCat->getItem($arrParam,array('task'=>'admin-newscategory-info'));
				$category_name = $thisNewsCat['category_name'];
				$this->_messagesError = "You can't delete news category \"" . $category_name . "\". There're still "
											. $newsmembercount . " news of this group!";
			}
		}
		
		//----- validate cho truong hop xoa nhieu group
		if ($options['task'] == 'validate-delete-multi-newscategory') {
			if (count($arrParam['cid'] > 0)) {
				...
				foreach ($cid as $key=>$val) {
					...
					$newsmembercount = $tblNewsCat->getItem(array('id'=>$val),array('task'=>'count-newscategory-members'));

					if ($newsmembercount > 0) {
						$thisNewsCat = $tblNewsCat->getItem(array('id'=>$val),array('task'=>'admin-newscategory-info'));
						$category_name = $thisNewsCat['category_name'];
						$this->_messagesError[$val] = "You can't delete news category \"" . $category_name . "\". There're still "
													. $newsmembercount . " news of this category!";
					} else {
