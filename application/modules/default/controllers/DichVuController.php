<?php
class DichVuController extends Zendvn_Controller_Action {
	...
	public function indexAction() {
		...
		//----- loc lay tat ca cac dich vu thuoc mot category (thuoc truc tiep hoac thuoc mot sub category nao do)
		if (!isset($this->_arrParam['id'])) {
			//----- neu khong co tham so id thi hien thi tat ca dich vu
		...
		$thisCategory = $tblServiceCat->getItem(array('id'=>$category_id),array('task'=>'default-servicecategory-info'));
		$this->view->thisCategory = $thisCategory;
		
		//----- lay danh sach cac item thuoc category nay
		$tblServices = new Default_Model_Services();
		$serviceList = $tblServices->listItem($this->_arrParam,array('task'=>'default-service-list-filter-to-category'));
		$this->view->serviceList = $serviceList;
		
		//----- lay tong so phan tu de phan trang
		$totalService = $tblServices->countItem($this->_arrParam,array('task'=>'count-item-in-ancestor-category'));
		$paginator = new Zendvn_Paginator();
		$paginator = $paginator->createPaginator($totalService, $this->_paginator);
		$this->view->paginator = $paginator;
	}
	
	public function dichVuChiTietAction() {
		$template_path = TEMPLATE_PATH . "/cnt/system";
		$this->loadTemplate($template_path,'template.ini','service-detail');
		
		$tblService = new Default_Model_Services();
		$this->view->Item = $tblService->getItem($this->_arrParam,array('task'=>'default-service-info'));
